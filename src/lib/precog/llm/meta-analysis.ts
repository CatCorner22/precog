...
export function runMetaAnalysis(profile: PracticeProfile): MetaAnalysisReport {
  ...
  for (const gap of knownUnknownItems(words, decisions.length)) {
    items.push({ ...gap, classification: "known_unknown" });
  }

  for (const gap of unknownUnknownItems(words)) {
    items.push({ ...gap, classification: "unknown_unknown" });
  }
