...
{"headline":"one sentence verdict","grade":"A|B|C|D|F",
...
Rules: 2-4 points per section, each under 200 characters, name specific processes in quotes. Grade should be consistent with the health score. Recommended moves must be doable by a small team this month.`;
  ...
  try {
    parsed = JSON.parse(text.replace(/^```(?:json)?/m, "").replace(/```$/m, "")) as Record<string, unknown>;
  } catch {
    return null;
  }
  const sections = (Array.isArray(parsed.sections) ? parsed.sections : [])
    .map((s) => {
      ...
      return {
        heading: String(sec.heading ?? "").trim().slice(0, 60),
  ...
  const grade = GRADES.has(String(parsed.grade))
    ? (String(parsed.grade) as MapReview["grade"])
    : gradeFromScore(input.health.score);
  return {
    source: "grok",
    model: body.model ?? "grok-4.5",
    headline: String(parsed.headline ?? "").trim().slice(0, 200) || "Grok reviewed your map.",
    grade,
    sections,
    nextMove: String(parsed.nextMove ?? "").trim().slice(0, 240) || "Open the hottest process and decide how to treat its risk.",
...
export const reviewMap = createServerFn({ method: "POST" })
  .validator((input: ReviewInput): ReviewInput => ({
    ...
    unownedProcesses: (input.unownedProcesses ?? []).map((s) => String(s).slice(0, 80)).slice(0, 10),
  }))
