...
const RISK_KINDS = new Set(["control", "fraud", "continuity", "quality", "compliance", "revenue", "safety"]);
...
function sanitizeRisk(r: Record<string, unknown>): SuggestedRisk | null {
  ...
  const kind = RISK_KINDS.has(String(r.kind)) ? (String(r.kind) as SuggestedRisk["kind"]) : "control";
  return {
    title,
    kind,
...
function sanitizeIdea(i: Record<string, unknown>): SuggestedIdea | null {
  const title = String(i.title ?? "").trim().slice(0, 80);
  if (!title) return null;
...
Rules: 3-4 risks, 2-3 ideas, 0-3 controlIds. Plain English an owner understands. Do not repeat already-listed items. Never accuse people; describe control gaps. Notes under 160 characters.`;
  ...
  return {
    ...
    ideas,
    controlIds,
    rationale: String(parsed.rationale ?? "").slice(0, 240) || "Grok reviewed the process description.",
  };
}
...
export const suggestForProcess = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<SuggestionResult> => {
    ...
    if (!apiKey || !data.processName.trim()) return local;
    try {
