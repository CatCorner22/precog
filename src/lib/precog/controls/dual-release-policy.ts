/**
 * Dual-release policy: two distinct people (or an owner override) above a
 * threshold for ACH, checks, write-offs, vendor master, deposits and payroll,
 * with time-bound exceptions by payee, person, role, channel or amount band.
 * This module holds the policy's shape, its defaults for a template, and the
 * merge that keeps a saved policy valid. Evaluation and summaries are beside it.
 *
 * Educational control design — not bank or accounting-system integration.
 */
 ...
 */
export const CHANNEL_SEATS: Record<
  ReleaseChannel,
...
export function holdsAny(person: Person, duties: readonly EntitlementId[]): boolean {
...
export function seatedByDuty(person: Person): boolean {
