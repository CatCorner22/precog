...
describe("coverage planner", () => {
  it("gives a 26-person restaurant the same backup program it gave before the speed-up", () => {
    ...
    expect(program.steps.map((s) => `${s.id}:${s.continuityGain}`)).toEqual([
      ...
      "prepare_deposit:own-16:3",
      "post_adjustments:own-17:1",
      "create_vendor:own-10:3",
      "release_payment:own-18:2",
      "edit_payroll_master:own-19:2",
      "post_journal_entries:own-20:1",
      "pms_admin_roles:own-9:2",
      "issue_refunds:own-21:2",
      "change_fee_schedule:own-7:2",
      "export_bulk_data:own-22:2",
      "enter_invoices:own-23:2",
      "initiate_ach:own-24:2",
    ]);
    ...
    expect(program.nextAssignments.map((p) => `${p.personId}=${p.entitlements.join("+")}`)).toEqual(
      [
        ...
        "own-7=collect_cash+view_reports_only+change_fee_schedule",
        "own-8=collect_cash+view_reports_only+review_audit_logs",
        "own-9=collect_cash+view_reports_only+pms_admin_roles",
        "own-10=collect_cash+view_reports_only+create_vendor",
        ...
        "own-16=view_reports_only+post_adjustments+prepare_deposit",
        "own-17=view_reports_only+create_vendor+post_adjustments",
        "own-18=collect_cash+view_reports_only+release_payment",
        "own-19=collect_cash+view_reports_only+edit_payroll_master",
        "own-20=view_reports_only+edit_payroll_master+post_journal_entries",
        "own-21=view_reports_only+pms_admin_roles+issue_refunds",
        "own-22=collect_cash+view_reports_only+export_bulk_data",
        "own-23=view_reports_only+issue_refunds+enter_invoices",
        "own-24=collect_cash+view_reports_only+initiate_ach",
  ...
  it("offers a 26-person restaurant the same three backups per weak duty it offered before", () => {
  ...
  it("shows each duty's conflict counts on the Power map exactly as a whole-team scan would", () => {
    ...
    for (const team of [restaurant26(), ...randomTeams(99, 2, 6)]) {
      for (const person of team) {
        const effects = dutyToggleEffects(person, duties);
