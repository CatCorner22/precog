...
describe("rankFirstSteps", () => {
  it("puts the controls that answer this business's findings ahead of pool-wide counts", () => {
    ...
    // Across the whole case pool the company-card review leads by case count.
    expect(steps[0].control.id).toBe("card-statement-line-review");
    const ranked = rankFirstSteps(steps, clinic);
    expect(ranked[0].control.id).not.toBe("card-statement-line-review");
    expect(ranked[0].answers).toBeGreaterThan(0);
    const card = ranked.findIndex((s) => s.control.id === "card-statement-line-review");
    for (const s of ranked.slice(0, card)) expect(s.answers).toBeGreaterThan(0);
    expect(ranked[card].answers).toBe(0);
