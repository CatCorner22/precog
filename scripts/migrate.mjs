...
const production =
  process.env.VERCEL_ENV === "production" || process.env.PRECOG_RUNTIME_MODE === "production";
...
async function main() {
  if (!databaseUrl) {
    if (production)
      throw new Error("DATABASE_MIGRATION_URL is required for a production migration");
  ...
  if (production && process.env.PRECOG_MIGRATIONS_APPROVED !== "true") {
    throw new Error(
      "Production migrations require separate release approval and PRECOG_MIGRATIONS_APPROVED=true",
    );
  }
  ...
  if (!loopback && !["direct", "session"].includes(process.env.MIGRATION_CONNECTION_MODE ?? "")) {
    throw new Error(
      "Confirm MIGRATION_CONNECTION_MODE=direct or session for the migration connection",
    );
  }
  ...
  try {
    ...
    do {
      const { rows } = await client.query("select pg_try_advisory_lock($1, $2) as acquired", [
        LOCK_NAMESPACE,
        LOCK_RESOURCE,
      ]);
      ...
      if (Date.now() >= deadline)
        throw new Error(
          "Another migration runner holds the lock; no migration was applied by this process",
        );
