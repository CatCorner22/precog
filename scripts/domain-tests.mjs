...
try {
  ...
  await test("value evidence is bounded, deduplicated, and summarized from verified records", () => {
    const records = valueEvidence.normalizeValueEvidence([
      { id: "one", kind: "recovery", description: "Recovered duplicate", source: "AP-1", amount: 500, observedAt: "2026-09-20", verified: true, injected: true },
      { id: "one", kind: "recovery", description: "duplicate", amount: 999 },
      { id: "two", kind: "time", description: "Review time", amount: -5, observedAt: "invalid", verified: false },
    ...
    assert.deepEqual(valueEvidence.summarizeValueEvidence(records), { total: 2, verified: 1, recoveries: 500, hours: 0, completion: 50 });
    ...
    assert.deepEqual(valueEvidence.assessEvidenceQuality(records, new Date("2026-09-20T00:00:00.000Z")), { unsourced: 1, stale: 1, future: 0, verified: 1, score: 50 });
    const unsourcedVerified = valueEvidence.normalizeValueEvidence([{ id: "unsafe", kind: "control", description: "No source", source: "", amount: 1, observedAt: "2026-09-20", verified: true }]);
    assert.equal(unsourcedVerified[0].verified, false);
    const impossibleDate = valueEvidence.normalizeValueEvidence([{ id: "date", kind: "control", description: "Bad date", source: "ticket", amount: 1, observedAt: "2026-99-99", verified: true }]);
    assert.equal(impossibleDate[0].observedAt, "");
    const futureQuality = valueEvidence.assessEvidenceQuality([{ ...records[0], observedAt: "2026-09-21" }], new Date("2026-09-20T00:00:00.000Z"));
    assert.deepEqual(futureQuality, { unsourced: 0, stale: 0, future: 1, verified: 0, score: 0 });
    const serialized = valueEvidence.serializeValueEvidence(records, new Date("2026-09-20T00:00:00.000Z"));
    assert.deepEqual(valueEvidence.parseValueEvidence(serialized), records);
    assert.throws(() => valueEvidence.parseValueEvidence('{"version":2,"evidence":[]}'), /Unsupported evidence version/);
  ...
  await test("value cases separate observed value from modeled avoided loss", () => {
    const result = valueCase.calculateValueCase({
      reviewHoursBefore: 30, reviewHoursAfter: 10, hourlyCost: 100, annualReviews: 4,
      directRecoveries: 2_000, annualExposure: 500_000, eventProbability: 0.02,
    ...
    const memoWithEvidence = valueCase.createValueCaseMemo(result.inputs, new Date("2026-01-01T00:00:00.000Z"), [{ id: "e1", kind: "recovery", description: "Vendor | refund", source: "AP-7", amount: 500, observedAt: "2026-01-01", verified: true }]);
    ...
    assert.equal(valueCase.applyVerifiedAnnualHours({ ...result.inputs, annualReviews: 0 }, 40).reviewHoursAfter, result.inputs.reviewHoursAfter);
  });

  await test("value case inputs are finite and conservatively bounded", () => {
    const normalized = valueCase.normalizeValueCase({
      reviewHoursBefore: -10, reviewHoursAfter: Number.NaN,
      annualExposure: Number.POSITIVE_INFINITY, eventProbability: 8,
  ...
  await test("snapshot comparison reports profile, assignment, and observed-value movement", () => {
    ...
    const currentProfile = profile.normalizeProfile({ ...archivedProfile, staff: { ...archivedProfile.staff, teamSize: archivedProfile.staff.teamSize + 2 }, riskVariables: { ...archivedProfile.riskVariables, deductible: archivedProfile.riskVariables.deductible + 1_000 } });
    const archivedMap = sodDetect.buildAssignments();
    const currentMap = archivedMap.map((item, index) => index === 0 ? { ...item, entitlements: [...item.entitlements, "manage_backups"] } : item);
  ...
  await test("power map covers common jobs and valid duty relationships", () => {
    ...
    for (const entitlement of sodRules.ENTITLEMENTS) {
      ...
      assert.ok(powerGuidance.POWER_GUIDANCE[entitlement.id]?.evidence, `${entitlement.id}:evidence`);
      assert.ok(powerGuidance.POWER_GUIDANCE[entitlement.id]?.boundary, `${entitlement.id}:boundary`);
    }
  });

  await test("family heuristics do not create cross-process false positives", () => {
    const assignments = [{
      personId: "cross-process", personName: "Cross Process", role: "Test",
  ...
  await test("conflict identity is invariant to entitlement order", () => {
    const forward = [{ personId: "ordered", personName: "Ordered", role: "Test", entitlements: ["collect_cash", "prepare_deposit", "post_payments"] }];
    ...
    const forwardCell = forwardReport.matrix.find((item) => item.row === "collect_cash" && item.col === "prepare_deposit");
    const reverseCell = forwardReport.matrix.find((item) => item.row === "prepare_deposit" && item.col === "collect_cash");
  ...
  await test("coverage analysis exposes ownership gaps and continuity risk", () => {
    ...
    assert.ok(baseline.duties.every((duty) => ["unassigned", "single_point", "covered"].includes(duty.status)));
  });

  await test("absence stress tests identify work that stops and lost backups", () => {
    const assignments = sodDetect.buildAssignments();
    const person = assignments.find((item) => item.entitlements.some((id) => {
    ...
    assert.ok(impact.remainingResilienceScore <= coverageAnalysis.analyzeDutyCoverage(assignments).resilienceScore);
  ...
  await test("power-map imports are bounded and allow-listed", () => {
    const normalized = modelIo.normalizeRoleAssignments({ assignments: [{
      personId: " person-1 ", personName: "A".repeat(200), role: "Manager",
      entitlements: ["collect_cash", "collect_cash", "not-a-power"], injected: true,
    }] });
    ...
    assert.equal(modelIo.normalizeRoleAssignments([{ personId: "x", personName: "X", role: "R", entitlements: [] }, { personId: "x", personName: "Y", role: "R", entitlements: [] }]), undefined);
  });
  ...
  await test("assignment previews match post-change conflict results", () => {
    ...
    const entitlement = sodRules.ENTITLEMENTS.find((item) => !person.entitlements.includes(item.id) && item.id !== "view_reports_only");
    assert.ok(entitlement);
    const impact = changeImpact.evaluateAssignmentChange(assignments, person.personId, entitlement.id);
    const before = sodDetect.detectSodConflicts(undefined, { assignments });
    const after = sodDetect.detectSodConflicts(undefined, { assignments: impact.nextAssignments });
    assert.equal(impact.action, "assign");
    assert.equal(after.conflicts.length - before.conflicts.length, impact.conflictsCreated.length - impact.conflictsResolved.length);
    const reverse = changeImpact.evaluateAssignmentChange(impact.nextAssignments, person.personId, entitlement.id);
    assert.equal(reverse.action, "remove");
    assert.deepEqual(reverse.nextAssignments, assignments);
    assert.equal(changeImpact.evaluateAssignmentChange(assignments, "missing", entitlement.id), undefined);
  });
  ...
  await test("assignment previews honor practice-specific scoring inputs", () => {
    ...
    const person = assignments[0];
    const entitlement = sodRules.ENTITLEMENTS.find((item) => !person.entitlements.includes(item.id) && item.id !== "view_reports_only");
    const staff = { ...profile.defaultProfile().staff, segregationScore: 20, dualControlPayments: false, independentBankRec: false };
    const impact = changeImpact.evaluateAssignmentChange(assignments, person.personId, entitlement.id, staff);
    ...
    assert.equal(impact.sodHealthChange, after.summary.segregationHealth - before.summary.segregationHealth);
  });

  await test("every matrix duty can be toggled for every modeled person", () => {
    const assignments = sodDetect.buildAssignments();
    for (const person of assignments) {
      for (const entitlement of sodRules.ENTITLEMENTS.filter((item) => item.id !== "view_reports_only")) {
        const impact = changeImpact.evaluateAssignmentChange(assignments, person.personId, entitlement.id);
        assert.ok(impact);
        const changed = impact.nextAssignments.find((item) => item.personId === person.personId);
        assert.equal(changed.entitlements.includes(entitlement.id), !person.entitlements.includes(entitlement.id));
      }
    }
  });
  ...
  await test("continuity planner recommends only conflict-free coverage improvements", () => {
    ...
    const beforeConflicts = sodDetect.detectSodConflicts(undefined, { assignments }).conflicts.length;
    ...
    for (const plan of plans) {
      ...
      const afterConflicts = sodDetect.detectSodConflicts(undefined, { assignments: plan.nextAssignments }).conflicts.length;
  ...
  await test("continuity program safely sequences interacting recommendations", () => {
    ...
    const afterConflicts = sodDetect.detectSodConflicts(undefined, { assignments: program.nextAssignments }).conflicts.length;
    ...
    for (const step of program.steps) counts.set(step.entitlement, (counts.get(step.entitlement) ?? 0) + 1);
  ...
  await test("governance report reconciles to live SoD and continuity results", () => {
    ...
    const report = governanceReport.createGovernanceReport(assignments, undefined, new Date("2026-09-19T00:00:00Z"));
  ...
  await test("assignment change review detects grants, revocations, hires, and removals", () => {
    const baseline = [{ personId: "a", personName: "Alex", role: "Manager", entitlements: ["collect_cash", "post_payments"] }];
    const current = [
      { personId: "a", personName: "Alex", role: "Manager", entitlements: ["collect_cash", "bank_reconcile"] },
    ...
    assert.deepEqual(new Set(changes.map((item) => item.kind)), new Set(["duty_granted", "duty_revoked", "person_added"]));
    assert.equal(assignmentDiff.diffAssignments(current, baseline).some((item) => item.kind === "person_removed"), true);
  ...
  await test("authority concentration index is bounded, complete, and deterministic", () => {
    ...
    assert.ok(ranked.every((item, index) => index === 0 || ranked[index - 1].authorityIndex >= item.authorityIndex));
    assert.deepEqual(powerIndex.calculatePowerIndex(assignments), ranked);
    const exclusive = powerIndex.calculatePowerIndex([{ personId: "solo", personName: "Solo", role: "Owner", entitlements: ["collect_cash"] }]);
